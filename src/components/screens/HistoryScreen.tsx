    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 border-b border-slate-200 dark:border-white/10 pb-4">
        ...
        <button
          ...
          className="bg-[#04A8FC] hover:bg-[#008fe0] text-white px-4 py-2.5 rounded-xl font-headline font-black text-xs uppercase flex items-center gap-2 shadow-md shadow-[#04A8FC]/20 transition-all hover:scale-[1.02] active:scale-95"
        >
      ...
      {/* Tabs Navigation */}
      <div className="flex items-center gap-2 p-1.5 bg-slate-200/80 dark:bg-white/5 rounded-2xl w-fit">
        <button
          onClick={() => {
            soundManager.playClick();
            setActiveTab('hall_of_fame');
          }}
          className={`px-5 py-2.5 rounded-xl font-headline font-black text-xs uppercase flex items-center gap-2 transition-all ${
        ...
        >
          ...
          <span>Encuentros Casuales ({casualEncounters.length})</span>
        </button>
      </div>
      ...
      {activeTab === 'hall_of_fame' && (
        activeBladers.length === 0 ? (
          ...
          <div className="glass-panel p-5 rounded-3xl border border-amber-500/40 bg-gradient-to-b from-amber-500/10 to-transparent shadow-sm space-y-4">
            ...
            <div className="space-y-2">
              {sortedByWins.slice(0, 5).map((b, i) => (
                <div
                  key={b.id}
                  className="flex items-center justify-between p-2.5 rounded-xl bg-white/70 dark:bg-white/5 border border-slate-200/50 dark:border-white/5 text-xs font-label-caps uppercase"
                >
                  <span className="font-bold text-slate-900 dark:text-white">
                    #{i + 1} {b.name}
                  </span>
                  <span className="font-headline font-black text-amber-500">
                    {b.stats.wins} Wins
                  </span>
                </div>
              ))}
            ...
            <div className="space-y-2">
              {sortedByXtreme.slice(0, 5).map((b, i) => (
                ...
                >
                  ...
                  <span className="font-headline font-black text-[#04A8FC]">
          ...
          {/* Most Burst Finishes */}
          <div className="glass-panel p-5 rounded-3xl border border-[#DC2626]/40 bg-gradient-to-b from-[#DC2626]/10 to-transparent shadow-sm space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-[#DC2626] text-white flex items-center justify-center font-bold shadow-md">
            ...
            <div className="space-y-2">
              {sortedByBurst.slice(0, 5).map((b, i) => (
                ...
                >
                  ...
                  <span className="font-headline font-black text-[#DC2626]">
          ...
          {/* Most Points Scored */}
          <div className="glass-panel p-5 rounded-3xl border border-purple-500/40 bg-gradient-to-b from-purple-500/10 to-transparent shadow-sm space-y-4">
            <div className="flex items-center gap-3">
            ...
            <div className="space-y-2">
              {sortedByPoints.slice(0, 5).map((b, i) => (
                ...
                >
                  ...
                  <span className="font-headline font-black text-purple-500">
          ...
          {officialTournaments.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {officialTournaments.map((t) => (
                <div
                  key={t.id}
                  className="glass-panel p-6 rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#15151c] shadow-sm hover:border-amber-500/50 transition-all space-y-4 relative group"
                >
                  ...
                  <div className="p-3.5 rounded-2xl bg-gradient-to-r from-amber-500/15 via-amber-500/5 to-transparent border border-amber-500/30 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-amber-500 text-black flex items-center justify-center font-bold shadow">
                      ...
                      <div>
                        ...
                        <h5 className="font-headline font-black text-base uppercase text-slate-900 dark:text-white">
                          {t.winnerName}
                        </h5>
                      </div>
                    </div>
                  ...
                  {/* Summary Stats */}
                  <div className="grid grid-cols-3 gap-2 text-center text-xs font-label-caps uppercase pt-1">
                    <div className="p-2 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5">
                      <span className="text-slate-400 block text-[9px]">Participantes</span>
                      <span className="font-black text-slate-900 dark:text-white">{t.totalBladers} Bladers</span>
                    </div>
                    <div className="p-2 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5">
                      <span className="text-slate-400 block text-[9px]">Combates</span>
                      <span className="font-black text-slate-900 dark:text-white">{t.totalMatches} Duelos</span>
                    </div>
                    <div className="p-2 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5">
                      <span className="text-slate-400 block text-[9px]">Puntos Totales</span>
                      <span className="font-black text-[#04A8FC]">{t.totalPoints} Pts</span>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full pt-2 border-t border-slate-100 dark:border-white/5 flex items-center justify-center gap-2 text-xs font-headline font-bold uppercase text-[#04A8FC] hover:text-[#008fe0] transition-colors py-1.5"
                  >
          ...
          {casualEncounters.length === 0 ? (
            <div className="glass-panel p-12 rounded-3xl border border-dashed border-slate-300 dark:border-white/10 text-center space-y-3 bg-white dark:bg-[#15151c]">
              ...
              <button
                ...
                className="bg-[#04A8FC] hover:bg-[#008fe0] text-white px-5 py-2 rounded-xl font-headline font-bold text-xs uppercase"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {casualEncounters.map((c) => (
                <div
                  key={c.id}
                  className="glass-panel p-5 rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#15151c] shadow-sm hover:border-[#04A8FC] transition-all space-y-3 relative group"
                >
                  ...
                  <h5 className="font-headline font-bold text-sm uppercase text-slate-900 dark:text-white">
                    {c.title}
                  </h5>

                  {c.matchesSummary && c.matchesSummary.length > 0 ? (
                    <div className="p-3 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 flex items-center justify-between font-headline font-bold uppercase text-xs">
                      <span className={c.matchesSummary[0].winner === c.matchesSummary[0].bladerA ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-600 dark:text-slate-400'}>
                        {c.matchesSummary[0].bladerA} ({c.matchesSummary[0].scoreA})
                      </span>
                      <span className="text-slate-400 font-mono text-[10px]">VS</span>
                      <span className={c.matchesSummary[0].winner === c.matchesSummary[0].bladerB ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-600 dark:text-slate-400'}>
                        {c.matchesSummary[0].bladerB} ({c.matchesSummary[0].scoreB})
                      </span>
                    </div>
                  ) : (
                    <p className="text-xs font-headline font-bold uppercase text-emerald-500">
      ...
      {selectedTournamentRecap && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in overflow-y-auto">
          <div className="relative w-full max-w-3xl bg-white dark:bg-[#15151c] rounded-3xl border border-slate-200 dark:border-white/10 shadow-2xl overflow-hidden p-6 md:p-8 space-y-6 my-8">
            {/* Header */}
            <div className="flex items-start justify-between gap-4 border-b border-slate-200 dark:border-white/10 pb-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="px-2.5 py-0.5 rounded-full bg-amber-500/10 text-amber-500 border border-amber-500/20 text-[10px] font-label-caps uppercase font-black">
                    {selectedTournamentRecap.season || 'Temporada Oficial'} • {selectedTournamentRecap.date}
                  </span>
                  <span className="px-2.5 py-0.5 rounded-full bg-[#04A8FC]/10 text-[#04A8FC] border border-[#04A8FC]/20 text-[10px] font-label-caps uppercase font-bold">
                ...
                <h2 className="font-headline font-black text-2xl md:text-3xl text-slate-900 dark:text-white uppercase tracking-tight">
              ...
              <button
                ...
                className="w-9 h-9 rounded-full bg-slate-100 dark:bg-white/10 text-slate-500 hover:text-slate-900 dark:hover:text-white flex items-center justify-center transition-colors"
              >
            ...
            <div className="p-3.5 rounded-2xl bg-amber-50 dark:bg-amber-950/20 border border-amber-300 dark:border-amber-500/30 flex items-center gap-3 text-xs text-amber-800 dark:text-amber-300 font-label-caps uppercase">
              <span className="material-symbols-outlined text-lg text-amber-500 flex-shrink-0">lock</span>
              <span>
                <strong>Registro Histórico de Torneo (Solo Lectura):</strong> Todos los resultados y puntuaciones son oficiales y definitivos para recapitulación.
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Champion Card */}
              <div className="glass-panel p-5 rounded-2xl border-2 border-amber-400/60 bg-gradient-to-r from-amber-500/20 via-yellow-500/10 to-transparent flex items-center gap-4 shadow-md">
                <div className="w-14 h-14 rounded-2xl bg-amber-500 text-black flex items-center justify-center font-black shadow-md flex-shrink-0">
                  <span className="material-symbols-outlined text-3xl">military_tech</span>
                </div>
                <div>
                  <span className="text-[10px] font-label-caps uppercase text-amber-600 dark:text-amber-400 font-black tracking-widest block">
                    🥇 Gran Campeón
                  </span>
              ...
              {/* Runner Up Card */}
              <div className="glass-panel p-5 rounded-2xl border border-slate-300 dark:border-white/10 bg-slate-50 dark:bg-white/5 flex items-center gap-4 shadow-sm">
                <div className="w-14 h-14 rounded-2xl bg-slate-300 dark:bg-slate-700 text-slate-800 dark:text-white flex items-center justify-center font-black shadow-sm flex-shrink-0">
                  <span className="material-symbols-outlined text-2xl">workspace_premium</span>
                </div>
                <div>
                  <span className="text-[10px] font-label-caps uppercase text-slate-500 font-bold tracking-widest block">
                    🥈 Subcampeón
                  </span>
                  <h3 className="font-headline font-black text-lg uppercase text-slate-800 dark:text-slate-200">
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10">
                <span className="text-slate-400 text-[10px] font-label-caps uppercase block">Participantes</span>
                <span className="font-headline font-black text-xl text-slate-900 dark:text-white">
                  {selectedTournamentRecap.totalBladers} Bladers
                </span>
              </div>
              <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10">
                <span className="text-slate-400 text-[10px] font-label-caps uppercase block">Combates Disputados</span>
                <span className="font-headline font-black text-xl text-slate-900 dark:text-white">
                  {selectedTournamentRecap.totalMatches} Duelos
                </span>
              </div>
              <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10">
                <span className="text-slate-400 text-[10px] font-label-caps uppercase block">Puntos Totales</span>
                <span className="font-headline font-black text-xl text-[#04A8FC]">
                  {selectedTournamentRecap.totalPoints} Pts
                </span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <h4 className="font-headline font-black text-sm uppercase text-slate-900 dark:text-white flex items-center gap-2">
                <span className="material-symbols-outlined text-base text-[#04A8FC]">sports_kabaddi</span>
                <span>Desarrollo de Combates y Resultados del Torneo</span>
              </h4>
              ...
              {selectedTournamentRecap.matchesSummary && selectedTournamentRecap.matchesSummary.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-72 overflow-y-auto pr-1">
                  {selectedTournamentRecap.matchesSummary.map((m, idx) => (
                    <div
                      key={idx}
                      className="p-3 rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 flex items-center justify-between gap-3 text-xs"
                    >
                      <div className="flex items-center gap-2 font-headline font-bold uppercase min-w-0">
                        <span className="w-5 h-5 rounded-full bg-slate-200 dark:bg-white/10 text-slate-700 dark:text-slate-300 flex items-center justify-center text-[10px] font-black flex-shrink-0">
                          #{idx + 1}
                        </span>
                        <div className="truncate">
                          ...
                          <span className="text-slate-400 mx-1.5 font-mono">VS</span>
                      ...
                      <div className="flex items-center gap-1 text-[10px] font-headline font-black uppercase text-amber-500 bg-amber-500/10 px-2 py-1 rounded-lg flex-shrink-0">
            ...
            <div className="flex justify-end pt-3 border-t border-slate-200 dark:border-white/10">
              <button
                ...
                className="bg-[#04A8FC] hover:bg-[#008fe0] text-white px-6 py-2.5 rounded-xl text-xs font-headline font-black uppercase tracking-wider shadow-md transition-all"
              >
