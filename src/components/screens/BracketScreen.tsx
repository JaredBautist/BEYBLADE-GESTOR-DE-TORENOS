      ...
      {matches.length === 0 ? (
        <div className="glass-panel p-8 sm:p-12 rounded-3xl border border-dashed border-[#bec7d3]/40 dark:border-white/10 text-center space-y-4 bg-white dark:bg-[#15151c]">
          ...
          <h3 className="font-headline font-black text-xl uppercase text-slate-900 dark:text-white">
            Árbol de Eliminación en Espera
          </h3>
          <p className="font-body-text text-xs sm:text-sm text-slate-500 max-w-md mx-auto">
            {bladers.length >= 2
              ? `Hay ${bladers.length} Bladers inscritos listos para competir. Genera el fixture del torneo para estructurar las rondas de eliminación.`
              : 'Inscribe al menos a 2 Bladers en el padrón para poder generar el árbol de eliminación del torneo.'}
          </p>
          {bladers.length >= 2 && (
            <button
              onClick={() => {
            ...
            >
              Generar Árbol con los {bladers.length} Bladers Inscritos
            </button>
          )}
        </div>
      ) : viewMode === 'rounds' ? (
