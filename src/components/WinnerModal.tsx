...
export const WinnerModal: React.FC<WinnerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white dark:bg-[#13131a] border-2 border-[#FCEE21] rounded-3xl max-w-lg w-full p-8 shadow-2xl relative overflow-hidden text-center space-y-6">
        {/* Glow background */}
        ...
        <div className="relative inline-block mx-auto">
          ...
          <div className="absolute -bottom-2 -right-2 w-9 h-9 rounded-full bg-[#FCEE21] border-2 border-white flex items-center justify-center text-black font-black shadow-md text-base">
            🏆
          </div>
        </div>
        ...
        <div>
          <span className="font-label-caps text-xs text-amber-500 uppercase tracking-widest font-black block mb-1">
            ¡VICTORIA CONFIRMADA! • {match.roundName}
          </span>
          <h2 className="font-headline font-black text-3xl md:text-4xl text-[#1a1c1e] dark:text-white uppercase tracking-tight">
            {winner.name}
          </h2>
          <p className="font-label-caps text-xs text-gray-500 uppercase mt-1">
            {winner.team} • {winner.combos[0]?.name}
          </p>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-black/5 dark:bg-white/5 border border-[#bec7d3]/30 dark:border-white/10 flex items-center justify-around">
          <div className="text-center">
            <span className="font-label-caps text-[10px] text-gray-500 uppercase block">
              {match.bladerA?.name}
            </span>
            <span className="font-headline font-black text-3xl text-[#AC191F]">
              {match.scoreA}
            </span>
          </div>

          <div className="font-headline font-bold text-lg text-gray-400">
            FINAL
          </div>
          ...
          <div className="text-center">
            ...
            <span className="font-headline font-black text-3xl text-[#04A8FC]">
              {match.scoreB}
            </span>
          </div>
        </div>
